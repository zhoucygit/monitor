...
### k8s部署
```
 kubectl  -n monitoring create configmap alertmanager-dingtalk-conf --from-file=template/template.tmpl 
 kubectl  -n monitoring create configmap alertmanager-dingtalk-conf --from-file=config/config.yaml
 kubectl  -n monitoring apply  -f k8syml/deployment.yaml
```
